...
| `milwaukee.html`'s `MKE_HEADLINES` block (inside the bundler template string) | `projects/life/milwaukee/calendar/upcoming.json` | `projects/life/milwaukee/calendar/refresh_map_events.py` (runs `scrape.py`, rewrites only the marked block, verifies the round-trip) | no — source and generator are outside the repo. The rest of the file is hand-built and bundled; never hand-edit inside the markers |
