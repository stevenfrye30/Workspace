...
| `milwaukee/data/news.json`, `milwaukee/data/events.json`, `milwaukee/data/observances.json` | the public RSS feeds of eleven local outlets; the calendars of the organizations in the hand-edited `milwaukee/data/orgs.json` and the citywide sources in `sources.json` (Tribe REST, ICS, schema.org JSON-LD, MLB's schedule API, UWM's per-department calendars, one page scraper); and UWM's multifaith calendar page for the observances | `tools/milwaukee/build.py` — rewrites a file only when its content changed; refuses to write an empty file over a full one. Run daily by `.github/workflows/milwaukee-desk.yml`. (The bundled press-map `milwaukee.html` and its `tools/milwaukee_map` / `tools/milwaukee_events` toolchain were retired 2026-09-13 — in git history before that date) | **YES** — tool, registry and workflow are tracked; the build needs only the public feeds. (`milwaukee/data/places.json` is the opposite case: seeded ONCE from UWM's "What's Around Campus" PDF by `tools/milwaukee/import_whatsaround.py --merge`, then hand-edited — never regenerated blindly, the Action never touches it) |
