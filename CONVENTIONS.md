...
| `milwaukee.html`'s `MKE_HEADLINES` block (inside the bundler template string) | live RSS via `tools/milwaukee_events/scrape.py` (intermediate `upcoming.json` is gitignored) | `tools/milwaukee_events/refresh_map_events.py` — rewrites only the marked block, verifies the round-trip, and leaves the file untouched when the headline list is unchanged. Run weekly by `.github/workflows/milwaukee-events.yml`; the old `projects/life/milwaukee/calendar/` copy is a forwarding stub | **YES** — tool and workflow are tracked; the scrape needs only the public feeds |
| `milwaukee.html`'s `MKE_SEED` block (same template string) | a personal data blob exported from the map's key | `projects/life/milwaukee/bake_map_data.py` | no — the blob is personal data outside the repo. Baked seed data is public on the live page; never hand-edit inside the markers |
| `milwaukee.html` as a whole (a bundler file — the app is one JSON template string) | hand-edited via `tools/milwaukee_map/unpack.py` → `_work/` → `pack.py` | `tools/milwaukee_map/bundle.py` holds the ONE copy of the splice/round-trip logic; every writer (pack, geocode_pins, the events refresher, the bake tool) goes through it. See that folder's README for the always-pull-first ritual | — (not generated; this row exists so nobody text-edits the bundle) |
