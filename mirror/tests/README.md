...
| `validate_v22_1_masonry.py` | The v22.1 spec's acceptance checks, A1–A7: the day chip reading the real date (today, `Yesterday · Aug 17`, backdated labels, the fake-clock midnight rollover, the ellipsis guard), masonry packing (the box below a short card starts one gap under it, every column-neighbour gap within one 8px row of the design gap, spans on every open card), drag + Alt+arrows + reload persistence over the packed grid, open/close and edit-mode re-packs, the density toggle's gap change, the single 375px column in user order, the hard internal slots unmoved, and the Checkups footer saying Today |

## Conventions

- A suite prints `PASS`/`FAIL` per check and exits non-zero on any failure.
- Ports are per-suite (8130–8144) so suites can run back to back.
